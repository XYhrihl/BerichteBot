	...
	public void readInput(String text, long id)
	{
		...
		if (expectUserName > 0 && expectIdList.contains(id))
		{
			if (getUserByName(text) == null)
			{
				...
				createUser (id, text);
				send = new SendMessage(id, "Hallo " + text + ". Dein Account wurde angelegt. Benutze den Befehl /about f�r weiter Informationen zu diesem Bot oder sende /help um einen �berblick �ber die Befehle zu sehen.");
			}
			else
			{
